...
from experiments import breast_cancer_reader, cardio_reader, credit_card_reader, phishing_reader, statlog_landsat_reader, \
                        obs_network_reader, telescope_reader, winequality_reader, mnist_reader, clave_reader
...
def bound_experiment(data, weak_signal_data, num_weak_signal, bound):
    ...
    print("Running tests...")
    learned_probabilities, optimized_weights = train_all(val_data, weak_signal_probabilities, bound, max_iter=10000)
...
def dependent_error_exp(data, weak_signal_data, num_weak_signal):
    ...
    print("Running tests...")
    learned_probabilities, optimized_weights = train_all(val_data, weak_signal_probabilities, weak_signal_ub, max_iter=5000)
    # calculate test probabilities
    test_probabilities = probability(test_data, optimized_weights)
    ...
    # Weighted average experiments
    ws_weights = (1 - weak_signal_ub) / np.sum(1 - weak_signal_ub)
    weighted_signals = np.average(weak_signal_probabilities, axis=0, weights=ws_weights)
    weighted_accuracy = getModelAccuracy(weighted_signals, val_labels)
    print("Weighted signal accuracy: ", weighted_accuracy)
    weighted_fit = LogisticRegression(solver="lbfgs", max_iter= 100)
    weighted_fit.fit(val_data.T, np.round(weighted_signals))
    weighted_accuracy = getModelAccuracy(weighted_fit.predict_proba(test_data.T)[:, 1], test_labels)
    print("Test accuracy: ", weighted_accuracy)
    ...
    output['W-AVG'] = weighted_accuracy
...
def run_experiment(data, weak_signal_data, num_weak_signal, constant_bound=False):
    ...
    #########################################
    # Run supervised experments
    # ws_weights = (1 - weak_signal_ub) / np.sum(1 - weak_signal_ub)
    # weighted_signals = np.average(weak_signal_probabilities, axis=0, weights=ws_weights)
    # weighted_accuracy = getModelAccuracy(weighted_signals, val_labels)
    # print("Weighted signal accuracy: ", weighted_accuracy)
    # supervised = LogisticRegression(solver="lbfgs", max_iter= 1000)
    # supervised.fit(val_data.T, val_labels)
    # supervised_accuracy = getModelAccuracy(supervised.predict_proba(test_data.T)[:, 1], test_labels)
    # print("Test accuracy: ",supervised_accuracy)
    # sys.exit(0)
    #########################################

    #########################################
    # filename = 'datasets/winequality/'
    # np.save(filename+'data_features.npy', val_data.T)
    # np.save(filename+'data_labels.npy', val_labels)
    # np.save(filename+'test_features.npy', test_data.T)
    # np.save(filename+'test_labels.npy', test_labels)
    # np.save(filename+'weak_signals.npy', weak_signal_probabilities.T)
    # sys.exit(0)
    #########################################

    print("Running tests...")
    if constant_bound:
        learned_probabilities, optimized_weights = train_all(val_data, weak_signal_probabilities, np.zeros(weak_signal_ub.size) + 0.3, max_iter=4000)
    else:
       learned_probabilities, optimized_weights = train_all(val_data, weak_signal_probabilities, weak_signal_ub, max_iter=4000)
...
def run_tests():
    ...
    # print("Running clave experiment...")
    # clave_reader.run_experiment(run_experiment, saveToFile)
    # print("Running credit card experiment...")
    # credit_card_reader.run_experiment(run_experiment, saveToFile)
    # print("Running statlog landsat experiment...")
    # statlog_landsat_reader.run_experiment(run_experiment, saveToFile)
    # print("Running phishing experiment...")
    # phishing_reader.run_experiment(run_experiment, saveToFile)
    # print("Running wine quality experiment...")
    # winequality_reader.run_experiment(run_experiment, saveToFile)
    # print("Running fashion-mnist experiment...")
    # mnist_reader.run_experiment(run_experiment, saveToFile)


    # print("Running bound experiment...")
    # obs_network_reader.run_bounds_experiment(bound_experiment)
    # statlog_landsat_reader.run_bounds_experiment(bound_experiment)
