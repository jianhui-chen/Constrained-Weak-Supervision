...
def read_crowdsourcing_data(datapath):
    ...
    # # crowd_labels = np.delete(crowd_labels, test_indexes, axis=1)
    # # train_indexes = np.delete(indexes, test_indexes)
    # # train_labels = train_labels[train_indexes]
    # # train_data = train_data[train_indexes]

    if len(crowd_labels.shape) == 2:
        n, m = crowd_labels.shape
        assert n >= m
        crowd_labels = crowd_labels.T.reshape(m, n, 1)

    data = {}
    data['train'] = train_data, train_labels
    data['test'] = test_data, test_labels
    data['weak_signals'] = crowd_labels
    return data
